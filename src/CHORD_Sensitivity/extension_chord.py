"""UV coverage and dirty-beam analysis for CHORD array-extension studies.

Provides helpers for:

- Loading antenna positions from UTM coordinate files
- Constructing :class:`pyuvdata.UVData` objects for interferometric simulations
- Plotting antenna layouts, UV coverage, and synthesized beams (dirty beams)
- Sweeping Briggs robustness to trade resolution against noise

This is a packaged port of the original ``notebooks/uv_utils.py`` script,
rewritten to follow the conventions of the rest of :mod:`CHORD_Sensitivity`
(relative imports, type annotations, shared physical constants).
"""
...
from pyuvdata import UVData, Telescope as UVTelescope
...
def utm_txt_to_earthlocations(filename: str, height_default: float = 545.0) -> EarthLocation:
...
def combine_locations(*loc_arrays: EarthLocation) -> EarthLocation:
...
def plot_uv_coverage(
    uvd: UVData, title: str = "", freq_hz: float = FREQ_HZ
...
def _measure_fwhm_1d(profile: NDArray, half_max: float = 0.5) -> float:
...
def _apply_weighting(
    ...
    u_idx: NDArray,
    valid: NDArray,
    weighting: Literal["natural", "uniform", "briggs"],
    robust: float | None,
    sum_w_natural: float,
    sum_w2_natural: float,
...
def compute_dirty_beam(
    uvd: UVData,
    title: str = "",
    npix: int = 128,
    freq_hz: float = FREQ_HZ,
    padding: float = 10.0,
    image_extent_deg: float | None = None,
    zoom_factor: float = 1.5,
    weighting: Literal["natural", "uniform", "briggs"] = "natural",
    ...
    plot_beam : bool
        Whether to render the dirty-beam figure.
    ...
    eff_l_arcmin, eff_m_arcmin : float
        Effective resolution in arcminutes from a 2-D Gaussian fit to the
        main lobe, along l and m.
    relative_noise : float
        Point-source noise relative to natural weighting.
    """
    lambda_m = C / freq_hz
